...
return(
    <div className="flex items-center justify-center min-h-screen px-6 sm:px-0 bg-gradient-to-br from-blue-200 to-purple-400">
        ...
        <div className="bg-slate-900 p-10 rounded-lg shadow-lg w-full sm:w-96 text-indigo-300 text-sm"> 
            <h2 className="text-3xl font-semibold text-white mb-3 text-center">{state === 'Sign Up' ? 'Create Account' : 'Login'}</h2>
            ...
            <form  onSubmit={onSubmitHandler}>
               {state === "Sign Up" && ( <div className="flex mb-4 items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]">
                ...
                {state !== 'Sign Up' && (<p  onClick={()=>navigate('/reset-password')} className="text-indigo-500 cursor-pointer mb-4">Forgot password?</p>)}
           ...
           {state === "Sign Up" ? ( <p className=" text-gray-500  text-center text-xs mt-4">Already have an account?{" "}<span onClick={()=>setState("Login")} className=" text-blue-500  cursor-pointer underline">Sign in</span></p> ) : 
                                  (<p className=" text-gray-500  text-center text-xs mt-4">Don't have an account? {" "} <span onClick={()=>setState("Sign Up")} className=" text-blue-500  cursor-pointer underline">Sign Up</span></p>)}
